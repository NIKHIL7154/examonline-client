...
import useCreateParticipants from "../hooks/useCreateParticipants";
...
function TestParticipants() {
    ...
    const handleListUpload = () => {
        ...
        toast.loading('Uploading participants list...',{duration:5000});
        ...
        const handleListAlteration = (id: string) => {
            ...
            if (isAdded) {
                setTestConfig(cur => {
                    return {
                        ...cur,
                        testParticipants: cur.testParticipants?.filter((participantId) => participantId !== id)
                    }
                })
            } else {
                setTestConfig(cur => {
                    return {
                        ...cur,
                        testParticipants: [...cur.testParticipants as string[], id]
                    }
                })
            }
        }
        return (
            <div>
                ...
                <div className="flex gap-8">
                    ...
                    <div className="w-1/2 h-[400px ] p-4 border-2 border-slate-300 rounded-lg overflow-y-auto">
                        <h3 className="text-xl font-semibold mb-4">Select Participants List</h3>
                        {createdParticipants ? (
                            <ul>
                                {/* Assuming testParticipants is an array of participant objects */}
                                {createdParticipants.map((participant, index) => (
                                    <li key={index} className={`mb-2 cursor-pointer  px-4 text-xl flex justify-between items-center h-12 border rounded-lg ${testParticipants?.includes(participant._id) ? 'bg-green-100' : 'bg-white'}`}
                                        onClick={() => {
                            ...
                            <p>No participants found. Please add a participant list.</p>
                        )}
                    </div>

                    {/* Add Participants Section */}
                    <div className="w-1/2 p-4 border-2 border-slate-300 rounded-lg">
                        <div className="text-xl font-semibold mb-4  flex justify-between"><h4>Add Participants</h4>

                        </div>
                        <div
                            className="w-full rounded-lg  gap-[0] h-[300px] p-4 border-2 border-dashed border-slate-300 hover:border-lime-700 flex justify-center items-center flex-col text-center"
