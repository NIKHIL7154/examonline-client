...
import BlankSetOptions from "./BlankSetOptions";
...
function TemplateList() {
    ...
    const handleOptionsWindow = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const uploadBox = target.closest('div[data-button]') as HTMLDivElement | null;
        ...
        if (uploadBox?.dataset.button === "upload-button") {
            return;
        }
    ...
    const createSetButton = (
        <li className="flex flex-col gap-3 ">
            <button data-button={"create-set-button"}>
                <div className={`
                ${expandOptions ? "border-green-500 text-7xl text-green-700" : "border-slate-200 text-green-900 text-8xl"} 
                h-[130px] w-[180px] border bg-white flex transition-[font] duration-100
                hover:text-green-700 hover:text-7xl border-2 hover:border-green-500`}>
    ...
    const renderedTemplates = templateDetails.map((el, i) => {
        return (
            <li className="flex flex-col gap-3" key={i + 1}>
                <button>
                    <div className="h-[130px] w-[180px] border border-slate-200 bg-white flex border-2 hover:border-green-500">
                        {el.image}
                    </div>
                </button>
    ...
    return (
        <div
            onClick={handleOptionsWindow}
            className=" h-auto p-[30px] flex flex-col justify-center items-center bg-gray-100 w-full"
        >
            <div className="w-[1000px] h-full gap-4 flex flex-col overflow-hidden ">
                ...
                {expandOptions && <BlankSetOptions expandUpload={expandUpload} />}
            </div>
        </div>
    );
}
